...
const CHART_ACCENT   = '#0D1B2A';
const CHART_DIM      = 'rgba(13,27,42,0.15)';
const CHART_GRID     = '#D8D0C4';
  ...
  return {
    ...
    options: {
      ...
      scales: {
        ...
        y: {
          ticks: { color: '#1C1C1C', font: { size: 12, weight: '500' } }
        }
      }
    }
  };
...
function initMap() {
  ...
  const markers = hotels.map(h => {
    const marker = L.circleMarker([h.lat, h.lng], {
      ...
      fillColor:   SEG_COLORS[h.category] || '#8A8070',
      color:       '#fff',
      weight:      1.2,
      opacity:     0.9,
      fillOpacity: 0.85,
...
function initTourismCharts() {
  ...
  function vBar(labels, values, bgColors, lblFmt, tipFmt) {
    return {
      type: 'bar',
      data: { labels, datasets: [{ data: values, backgroundColor: bgColors, borderRadius: 0, borderSkipped: false }] },
      options: {
        ...
        plugins: {
          legend: { display: false },
          tooltip: {
            backgroundColor: '#FFFFFF', borderColor: '#D8D0C4', borderWidth: 1,
            titleColor: '#1C1C1C', bodyColor: '#8A8070',
            padding: { top: 8, bottom: 8, left: 12, right: 12 },
            callbacks: { label: ctx => '  ' + tipFmt(ctx.raw) },
          },
          datalabels: {
            anchor: 'end', align: 'top', clip: false,
            color: '#1C1C1C', font: { size: 11, weight: '600' },
  ...
  new Chart(document.getElementById('chart-tour-transport'), {
    ...
    data: {
      ...
      datasets: [
        { label: 'Air',  data: [7.2, 9.8, 12.1, 14.2, 15.8], backgroundColor: CHART_ACCENT, borderRadius: 0, borderSkipped: false },
        { label: 'Sea',  data: [2.8, 3.2,  3.8,  4.1,  4.6], backgroundColor: '#3B6D11',    borderRadius: 0, borderSkipped: false },
        { label: 'Land', data: [1.0, 1.5,  1.5,  1.8,  2.1], backgroundColor: FC,           borderRadius: 0, borderSkipped: false },
      ],
    },
    options: {
      responsive: true, maintainAspectRatio: false, animation: { duration: 350 },
  ...
  new Chart(document.getElementById('chart-tour-season'), {
    ...
    data: {
      ...
      datasets: [{
        ...
        pointBorderColor: '#F5F0E8', pointBorderWidth: 2,
      }],
    },
    options: {
      responsive: true, maintainAspectRatio: false, animation: { duration: 350 },
      ...
      plugins: {
        ...
        datalabels: {
          ...
          color: '#1C1C1C', font: { size: 10, weight: '600' },
          formatter: v => v,
        },
        tooltip: {
          backgroundColor: '#FFFFFF', borderColor: '#D8D0C4', borderWidth: 1,
...
function initPipelineMap() {
  ...
  const markers = pipelineData.map(p => {
    const color = p.status === 'Under Construction' ? '#B8922A' : '#0D1B2A';
...
function pipelinePopupHTML(p) {
  ...
  const statusPill = p.status === 'Under Construction'
    ? `<span style="background:rgba(184,146,42,0.12);color:#854F0B;border:1px solid rgba(184,146,42,0.35);border-radius:2px;padding:1px 6px;font-size:10px;font-weight:700">${p.status}</span>`
    : `<span style="background:rgba(13,27,42,0.10);color:#0D1B2A;border:1px solid rgba(13,27,42,0.25);border-radius:2px;padding:1px 6px;font-size:10px;font-weight:700">${p.status}</span>`;
...
function renderPipelineCharts() {
  ...
  const projColors = sorted.map(p => p.status === 'Under Construction' ? '#B8922A' : '#0D1B2A');
...
function adrCellStyle(adr) {
  if (adr < 4000) return {bg:'#D8E4F0', col:'#0D1B2A'};
  if (adr < 6000) return {bg:'#3D5166', col:'#E8E8E0'};
  return {bg:'#0D1B2A', col:'#FFFFFF'};
}

function occCellStyle(occ) {
  const p = occ * 100;
  if (p < 60) return {bg:'#F5EBE0', col:'#854F0B'};
  if (p < 80) return {bg:'#8AB870', col:'#FFFFFF'};
  return {bg:'#3B6D11', col:'#FFFFFF'};
}
      ...
      data: {
        ...
        datasets: [
          { label: 'My Property', data: myData, borderColor: CHART_ACCENT,
            backgroundColor: 'rgba(13,27,42,0.07)', borderWidth: 2, fill: false,
...
function renderBenchDOW() {
  ...
  benchDOWChart = new Chart(document.getElementById('bench-dow-chart'), {
    ...
    data: {
      ...
      datasets: [
        { label: 'My Property', data: myDOW.map(avg), backgroundColor: CHART_ACCENT, borderRadius: 0, borderSkipped: false },
        { label: 'Comp Set Avg', data: compDOW.map(avg), backgroundColor: 'rgba(184,146,42,0.35)', borderRadius: 0, borderSkipped: false },
      ],
    },
    options: {
      responsive: true, maintainAspectRatio: false, animation: { duration: 300 },
